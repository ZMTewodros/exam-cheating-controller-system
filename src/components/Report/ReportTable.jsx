...
function ReportTable() {
  ...
  // Formats to string then safely splits text by the ' at ' segment
  function formatDateTime(timestamp) {
    if (!timestamp) return "--- at ---";
    const date = (timestamp instanceof Date) ? timestamp : new Date(timestamp);
    if (isNaN(date.getTime())) return "Invalid at Date";
  ...
  }, [logs, search, categoryFilter, riskFilter, roomSearch, departmentFilter, yearFilter, dateFilter, authorizedSsids]);

  const totalDevices = useMemo(() => filteredLogs.length, [filteredLogs]);
  const authorizedCount = useMemo(
    () => filteredLogs.filter((log) => authorizedSsids.includes(log.ssid)).length,
    ...
    <div className="max-w-7xl mx-auto px-4 space-y-6">
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        <div className="flex flex-col lg:flex-row justify-between gap-6">
          <div>
            <h2 className="text-3xl font-black text-slate-900">Reports & Logs</h2>
            <p className="text-slate-500 mt-2 font-medium">Review, filter and export device detection history</p>
          </div>
          <ExportCSV logs={filteredLogs} authorizedSsids={authorizedSsids} />
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="flex flex-wrap gap-4 mt-6">
          <button onClick={loadData} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-bold transition">
            Refresh Data
          </button>
          <button onClick={clearFilters} className="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-3 rounded-xl font-bold transition">
            Clear Filters
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b text-sm font-bold text-slate-700">
          Total Devices (Filtered): {totalDevices} | Authorized: {authorizedCount} | Unauthorized: {unauthorizedCount}
        </div>
          ...
          <table className="min-w-full text-left">
            ...
            <tbody>
              {loading ? (
                ...
                const dateTimeStr = formatDateTime(log.timestamp);

                return (
                  <tr key={log.id} className="hover:bg-slate-50 transition">
                    <td className="px-8 py-5 text-xs font-bold text-gray-500 whitespace-nowrap">
                      <div className="flex flex-col">
                        <span>{dateTimeStr.split(' at ')[0]}</span> 
                        <span className="text-blue-500 font-medium">{dateTimeStr.split(' at ')[1]}</span>
                      </div>
                    </td>
                    <td className="px-8 py-5 font-bold text-slate-800">{log.ssid || "Hidden"}</td>
                    <td className="px-8 py-5 text-xs font-mono text-gray-500">{log.mac}</td>
                    <td className="px-8 py-5 text-sm text-gray-600">{log.room || "Exam Hall A"}</td>
                    <td className="px-8 py-5 text-sm text-gray-600">{log.department || "-"}</td>
                    <td className="px-8 py-5 text-sm text-gray-600">{log.year || "-"}</td>
                    <td className="px-8 py-5">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                        isAuth ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                      }`}>
                        {isAuth ? "Authorized" : "Unauthorized"}
                      </span>
                    </td>
                    <td className="px-8 py-5 font-black text-slate-700">{signalVal} dBm</td>
                    <td className="px-8 py-5">
                      <span className={`font-black uppercase text-xs ${
                        isAuth ? "text-green-600" : risk === "High" ? "text-red-600" : risk === "Medium" ? "text-yellow-600" : "text-gray-500"
                      }`}>
                        {isAuth ? "Safe" : risk}
                      </span>
                    </td>
